// room-cam — Seeed XIAO ESP32S3 Sense · room security camera
//
...
//       - pushed to the room-watch cloud page (view from anywhere)
//       - first photo attached to an ntfy push notification on your phone
//       - archived to microSD when a card is present (ring buffer)
//   · "go live" from the cloud page: camera streams ~1.5 fps to the page
//     for 90s windows while you watch, from anywhere
//   · one frame every IDLE_FRAME_SECONDS keeps the away page fresh and
//     doubles as the camera's heartbeat
//
// Split across cores like bunny-hop: capture/motion on core 1, WiFi +
...
// POST one JPEG. The response carries the flags, so a streaming camera
// hears "stop"/"disarm"/"test" without separate polls.
...
static const char PAGE_TEMPLATE[] =
    ...
    "<title>room-cam</title><style>"
    "body{background:#0b0d10;color:#e8eaed;font-family:-apple-system,sans-serif;"
    "margin:0;padding:16px;max-width:560px;margin:0 auto}"
    "h3{letter-spacing:.2em;font-size:14px;margin:6px 2px 12px}"
    "img{width:100%;border-radius:14px;border:1px solid #262b33;background:#000}"
    ".row{display:flex;gap:10px;margin-top:12px}"
    "button,a.b{flex:1;padding:13px;border-radius:12px;border:1px solid #262b33;"
    "background:#14171c;color:#e8eaed;font-size:15px;font-weight:600;"
    "text-align:center;text-decoration:none;cursor:pointer}"
    "button.on{background:#143d24;color:#86efac;border-color:#22c55e55}"
    "pre{color:#8b93a1;font-size:12px;white-space:pre-wrap}"
    "</style></head><body><h3>ROOM CAM \xC2\xB7 LOCAL</h3>"
    "<img src=\"http://%IP%:81/stream?k=%K%\">"
    "<div class=row><button id=a onclick=arm()>\xE2\x80\xA6</button>"
    "<button onclick=\"fetch('/flip?k=%K%')\">Rotate 180\xC2\xB0</button>"
    "<a class=b href=\"https://%HOST%\">cloud \xE2\x86\x92</a></div>"
    "<pre id=s>loading\xE2\x80\xA6</pre><script>"
    "let armed=false;"
    "async function st(){try{const j=await(await fetch('/status.json?k=%K%')).json();"
    "armed=j.armed;document.getElementById('a').textContent=armed?'Armed \xE2\x9C\x93':'Disarmed';"
    "document.getElementById('a').className=armed?'on':'';"
    "document.getElementById('s').textContent="
    "'ip '+j.ip+' \xC2\xB7 wifi '+j.rssi+'dBm \xC2\xB7 sd '+(j.sd?'ok':'none')+"
    "' \xC2\xB7 cloud '+(j.cloudOkAgoS<0?'never':j.cloudOkAgoS+'s ago')+"
    "' \xC2\xB7 up '+Math.floor(j.uptimeS/60)+'m';}catch(e){}}"
...
static void netTask(void *) {
  ...
  int64_t lastIdleFrame = -(int64_t)IDLE_FRAME_SECONDS * 1000;
  ...
  for (;;) {
    ...
    if (!bootNotified) {
      ...
      ntfyPush("Room camera online", msg, "low", nullptr, 0);
    }
    ...
    if (bootMs() - lastIdleFrame >= (int64_t)IDLE_FRAME_SECONDS * 1000) {
      size_t len = 0;
      uint8_t *jpg = grabJpeg(&len);
      bool pushed = false;
      if (jpg) {
        pushed = pushFrame("idle", nullptr, 0, jpg, len);
        free(jpg);
      }
      // On failure retry in ~20s instead of hammering or going dark for 5min.
      lastIdleFrame = pushed
                          ? bootMs()
                          : bootMs() - (int64_t)IDLE_FRAME_SECONDS * 1000 + 20000;
    }
...
void setup() {
  ...
  Serial.printf("[room-cam] armed=%s\n", armed ? "yes" : "no");
}
